...
#[derive(Debug)]
pub struct Query {
    /// The table `FROM` named, which the request's own `tables` is asked for.
    pub table: Table,
...
/// The table a statement reads, as it was written.
#[derive(Debug, PartialEq, Eq)]
pub struct Table {
    pub name: String,
    /// `FROM gaia AS g`. Kept because it is what a qualified column reference in the rest of
    /// the statement is written against.
...
pub fn parse(query: &str, limits: sql::Limits) -> Result<Query, ApiError> {
    ...
    let select = select_list(selected.projection)?;
    if let Some(items) = &select {
        for item in items {
            no_geometry(&item.expr, "a select list")?;
        }
    }
    let (predicate, spatial) = match selected.selection {
        None => (None, None),
        Some(selection) => split(selection, &table)?,
    };
    Ok(Query {
        table,
        select,
...
fn split(
    selection: SqlExpr,
    table: &Table,
) -> Result<(Option<SqlExpr>, Option<Spatial>), ApiError> {
    ...
    for conjunct in conjuncts(selection) {
        ...
        spatial = Some(constraint(&conjunct, table)?);
    }
    Ok((residual, spatial))
}
...
fn constraint(expr: &SqlExpr, table: &Table) -> Result<Spatial, ApiError> {
    let mut regions = Vec::new();
    let mut columns: Option<(String, String)> = None;
    for disjunct in disjuncts(expr) {
        let (region, ra_column, dec_column) = region_test(disjunct, table)?;
        match &columns {
...
fn inside(call: &Function, table: &Table) -> Result<(Region, String, String), ApiError> {
    ...
    let (point, shape) = match (point_columns(first, table), symmetric) {
        (Ok(columns), _) => (columns, second),
        (Err(_), true) => (point_columns(second, table)?, first),
...
fn within(
    ...
    radius: f64,
    table: &Table,
) -> Result<(Region, String, String), ApiError> {
    let args = positional(call).ok_or_else(shape_of_a_region_test)?;
    let (columns, centre) = match args.as_slice() {
        [first, second] => match (point_columns(first, table), point_position(second)) {
            (Ok(columns), Some(centre)) => (columns, centre),
            _ => match (point_position(first), point_columns(second, table)) {
        ...
        [ra, dec, centre_ra, centre_dec] => match (
            ...
            _ => match (
                ...
                column_pair(centre_ra, centre_dec, table),
            ) {
...
fn point_columns(expr: &SqlExpr, table: &Table) -> Result<(String, String), ApiError> {
    let wrong_shape = || {
        ApiError::bad_request(format!(
...
/// Two column names, each written plainly or qualified by the table it came from.
///
/// A qualifier is stripped here and nowhere else. Elsewhere a dotted name is ambiguous —
/// this service reads `lightcurve.mag` as a projection into a struct column, which is a
/// headline feature, and which of the two a name is takes the file's schema to decide. A
/// coordinate is not that case: nothing here would read a struct as a position, so a head
/// matching the table's own name is the table's.
fn column_pair(ra: &SqlExpr, dec: &SqlExpr, table: &Table) -> Result<(String, String), ApiError> {
    let named = |expr: &SqlExpr| match unnest(expr) {
        ...
        SqlExpr::CompoundIdentifier(parts) => match parts.as_slice() {
            [head, column] if qualifies(head, table) => Some(column.value.clone()),
            _ => None,
        },
        _ => None,
    };
...
/// Whether a dotted name's head is the table this statement reads.
fn qualifies(head: &Ident, table: &Table) -> bool {
    let matches = |name: &str| {
        head.value == name || head.value.eq_ignore_ascii_case(name) && head.quote_style.is_none()
    };
    match &table.alias {
        Some(alias) => matches(alias),
        None => matches(&table.name),
    }
}
...
mod tests {
    ...
    fn a_statement_becomes_a_table_a_projection_and_a_limit() {
        ...
        assert_eq!(query.table.name, "gaia");
        assert_eq!(query.table.alias, None);
    ...
    fn the_table_keeps_the_spelling_it_was_written_in() {
        ...
        assert_eq!(parsed("SELECT * FROM GaiaDR3").table.name, "GaiaDR3");
        assert_eq!(parsed(r#"SELECT * FROM "gaia dr3""#).table.name, "gaia dr3");
    }
    ...
    #[test]
    fn a_qualified_position_names_the_same_columns() {
        let query = parsed(
            "SELECT g.source_id FROM gaia AS g \
             WHERE 1 = CONTAINS(POINT(g.ra, g.dec), CIRCLE(45.0, -20.0, 0.1))",
        );
        assert_eq!(query.table.alias.as_deref(), Some("g"));
        assert_eq!(
            query
                .spatial
