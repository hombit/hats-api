...
/// `tsv` and `parquet` carry this instead. It is an extension this service introduces and no
/// client reads it; what it is for is that the fact is stated somewhere rather than nowhere, a
/// delimited body having no room for it and TAP defining nothing for one. Parquet has a place
/// it could go — the footer's key/value metadata — and that is worse than nowhere: no reader
/// surfaces it, so a truncation said there is one a caller cannot find while the file looks
/// whole.
///
